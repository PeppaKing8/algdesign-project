...
\begin{lemma}
\label{apspext3}
In each iteration, at least $(1-\alpha)^{M}$ fraction of entries of $D$ in $L$ will be given a witness, where $M=1+3\log_{4/3} n$.
\end{lemma}
...
For the deterministic construction of $\mathcal{S}$, please refer to \cite{10.1145/100216.100244}. Here we provide a theoretical proof that such subset of this size exists:
...
Based on Algorithm, Karger speeds up the algorithm by a factor of $\Theta((\log n)^2)$ using a quite simple algorithm \cite{karger2000minimum}, relying only on finding the least common ancestors and evaluating expression-trees, and may
...
% and 
\begin{definition}
    A cut in $G$ is $\alpha$-minimum if its value is at most $\alpha$ times the minimum cut value.
\end{definition}
...
\begin{lemma}
  ...
  \frac{1}{\lfloor 2\alpha \rfloor + 1 -2\alpha} \binom{n}{\lfloor
      2\alpha \rfloor}\left(1+O\left(\frac{1}{n}\right)\right)
  \]
\end{lemma} 
...
\begin{proof}
    ...
    It takes $O(n)$ time to compute all $n$ cut values. We know extend our DP function from paths to general trees. For any function $f$, we define the treefix sum of $f$, denoted $\descendants{f}$, as
    \begin{align*}
...
\begin{proof}
    ...
    We already show how to compute all $\cutval{\descendants{v}}$ and $\descendants{\rho}(v)$ in $O(m)$-time, similarly $\crossweight{\descendants{v}}{\descendants{w}}$ can be computed using the treefix sum of $f_v(w)=\crossweight{v}{w}$ (which is the weight of edge $(v,w)$), then for each $v,w$, all $n^2$ values
    \begin{align*}
...
\begin{enumerate}
    \item \textbf{Linear Time Deterministic MST Algorithms:} While randomized algorithms, like Karger's, achieve near-linear time complexity, it is still an open question whether a deterministic linear-time algorithm exists for finding the MST in undirected, weighted graphs.
    \item \textbf{MST in Hypergraphs:} Extending the MST problem to hypergraphs, where edges can connect more than two vertices, is less understood and lacks efficient algorithms. Finding optimal algorithms for this generalization, including approximation methods, remains an open question.
...
\begin{enumerate}
    ...
    \item \textbf{EM} together with other related problems, such as \textbf{TkPM} (Top-k Perfect Matching) \cite{maalouly2022exact}, remains open to design a better \textbf{deterministic approximation algorithm}.
\end{enumerate}
...
Given a graph $G(V,E)$, and a set of points $P\subseteq V$, called \textbf{key points}, the \textsf{Minimum Point Traversing Path (MPTP)} problem is to find a path that traverses all points in $P$ \textbf{at least once}, such that the length of the path (number of edges) is minimized. 

In fact, \textsf{MPTP} is NP-complete. We can prove \textsf{Hamiltonian-Path} $\le_P$ \textsf{MPTP} by setting $P=V$.

An easier version of the problem is \textsf{Optimal Point Traversing Path (OPTP)}. For every $P'\subseteq V$, let $\xi(P')$ be minimum length of path that traverses all points in $P'$ at least once. For $1\le i\le n$, let
...
The \textsf{OPTP} problem requires to find an algorithm $\mathcal{A}$, such that
...
Now we give an algorithm $A$, that runs in $o(n^2)=o(|V|)$ time (i.e. \textbf{sublinear}), to solve \textsf{OPTP} on $G_{n+1}$. Using Lemma \ref{hahaha1}, we only need to show that $A$ outputs a path of length at most $\sqrt{2}n^{3/2}+10n$ in every case.
...
in the order of $x$-coordinate, i.e. $x_1\le x_2\le \cdots\le x_k$. The number of step in this process can be calculated as
\begin{align*}
V_{\text{left}\to \text{right}}(l,r):=n+\left(|l-y_1|+\sum\limits_{i=1}^{k-1} |y_{i}-y_{i+1}|+|y_k-r|\right)
\end{align*}
The reason is as follows: at $x$-coordinate, we always go in one direction, so the number of steps is $n$; at $y$-coordinate, from $v_i$ to $v_{i+1}$, we need $|y_i-y_{i+1}|$ steps, adding the contribution of $(l,0)\to v_1$ and $v_k\to (r,n)$, we get the above formula.

\begin{definition}
\label{def5.3}
For a point $A_i$, if it lies in a strip $[l,r)$, define the \textbf{distance function} $w(A_i)=\max\{y_i-l,r-y_i\}$.
\end{definition}
We use the Definition \ref{def5.3} to give a bound of $V_{\text{left}\to \text{right}}(l,r)$.
...
for an odd $T$,
$$\mathbb{E}[w(V)]=\dfrac{1}{T}\cdot \left(\dfrac{3T}{2}+1\right)\dfrac{T}{2}\le \dfrac{3T}{4}+1.$$
\end{proof}
...
Thus, we can try all possible $b$, and find the minimum length of the path. Combining Lemma \ref{hahaha1} and Theorem \ref{theo5.1}, $A$ is a solution to \textsf{OPTP} on grid graph $G_{n+1}$.
...
We believe that, if a graph has the similar property as the grid graph, we can also design a fast algorithm to solve the \textsf{OPTP} problem on it.
...
We would like to thank Prof. Jian Li and all TAs for their guidance and help in the course of the study. We also appreciate the authors of the papers we cited for their excellent work. Without their contributions, this survey would not have been possible.
