...
This report provides a comprehensive overview of randomized techniques applied to classic graph problems. We explore foundational problems such as All Pairs Shortest Path (APSP), Min Cut, and Minimum Spanning Tree (MST), examining both well-established and cutting-edge randomized approaches. For each problem, we discuss the theoretical underpinnings of the algorithms, assess their performance through time complexity analysis, and evaluate the potential for improvements and extensions to more complex graph types, including directed and weighted graphs. Additionally, we present open problems to encourage further research in this domain.
...
Note that if we see $f(a,b)$ as an individual indeterminate, the $x^k$ coefficient of Pffafian is a polynomial in all $f(a,b)$. When all indeterminate $f(a,b)$ are randomly chosen, we will bound the probability of success by Schwarz's Lemma.

\begin{lemma}
\label{schwarz}
Let $x_1,x_2,\cdots,x_n$ be $n$ indeterminates, and $P(x_1,x_2,\cdots,x_n)\in \mathbb{F}_p(x_1,x_2,\cdots,x_n)$ is a non-zero polynomial of degree $\le d$. Then,
